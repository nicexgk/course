...
function form_init() {
    layui.use(['element', 'upload', 'form', 'layer'], function () {
        ...
        form.on('submit(addcoursesubmit)', function (data) {
            ...
            course.course_name = $("input[name='coursename']").val();
            course.pic_url = $("input[name='photo']").val();
            course.course_type = $("#select3").val();
...
function sortSelect_1(index) {
    ...
    for (var parentType in courseTypeCatalog) {
        if (courseTypeCatalog[parentType].type_id == index) {
            var secondTypeList = courseTypeCatalog[parentType].subclassList;
            // 当没有子列表时设置其他选择框的内容为空
            ...
            for (var secondType in secondTypeList) {
                if(secondType == 0){
                    sortSelect_2(secondTypeList[secondType].type_id);
                }
                select_2.append('<option value="' + secondTypeList[secondType].type_id + '">' + secondTypeList[secondType].type_name + '</option>');
            }
            break;
        }
    }
...
function sortSelect_2(index_2) {
    ...
    for (parentType in courseTypeCatalog) {
        if (courseTypeCatalog[parentType].type_id == index) {
            secondTypeList = courseTypeCatalog[parentType].subclassList;
            break;
        }
    }
    console.log(secondTypeList);
    for (var secondType in secondTypeList) {
        if (secondTypeList[secondType].type_id == index_2) {
            var chileTypeList = secondTypeList[secondType].subclassList;
            for (var chileType in chileTypeList) {
                select_3.append('<option value="' + chileTypeList[chileType].type_id + '">' + chileTypeList[chileType].type_name + '</option>');
            }
        }
    }
}
