            ...
            <div class="layu-input-block">
                        ...
                        <c:forEach items="${requestScope.courseTypeCatalog }" var="parentType" varStatus="status">
                            <c:choose>
                                <c:when test="${status.index == 0}">
                                    <option value="${parentType.type_id}" selected>${parentType.type_name }</option>
                                </c:when>
                                <c:otherwise>
                                    <option value="${parentType.type_id}">${parentType.type_name }</option>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                    </select>
                    ...
                    <select id="select2" lay-filter="select2" name="second-type" style="width:170px;">
                        <c:forEach items="${requestScope.courseTypeCatalog }" var="parentType" varStatus="status" begin="0" end="0">
                            <c:forEach items="${parentType.subclassList}" var="secondType">
                                <option value="${secondType.type_id}" selected="selected" data-bind-parentId="${parentType.type_id}">${secondType.type_name }</option>
                            </c:forEach>
                        </c:forEach>
                    </select>
                </div>
                <div class="firstselect" id="div3">
                    <select id="select3" name="course-type" lay-filter="select3" style="width:170px;">
                        <c:forEach items="${requestScope.courseTypeCatalog }" var="parentType" varStatus="status" begin="0" end="0">
                            <c:forEach items="${parentType.subclassList}" var="secondType">
                                <c:forEach items="${secondType.subclassList}" var="chileType">
                                    <option value="${chileType.type_id}" data-bind-parentId="${secondType.type_id}">${chileType.type_name }</option>
                                </c:forEach>
                            </c:forEach>
                        </c:forEach>
                    </select>
