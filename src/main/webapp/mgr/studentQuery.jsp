...
<body>
	<form class="form-inline definewidth m20" action="#" method="get">
		<font color="#777777"><strong>学生姓名：</strong></font> <input type="text" name="menuname" id="menuname"
	...
	<table class="table table-bordered table-hover definewidth m10">
		...
		<c:forEach items="${requestScope.studentList }" var="temp">
			<tr>
				<td><a href="queryUserDetail?userId=${temp.userId }"><c:out value="${temp.userName }"></c:out></a></td>
				<td><c:out value="${temp.userPhone }"></c:out></td>
				<td><c:out value="${temp.userEmail }"></c:out></td>
				<td><c:out value="${temp.userDate }"></c:out></td>
			...
			$.getJSON("queryUserAjax", {
				...
				for ( var temp in data.userList) {
					str = str + '<tr>' + '<td><a href="teacherdetail.jsp">'
							+ data.userList[temp].userName + '</a></td>'
							+ '<td>' + data.userList[temp].userPhone + '</td>'
							+ '<td>' + data.userList[temp].userEmail + '</td>'
							+ '<td>' + data.userList[temp].userDate + '</td>'
							+ '<td><button type="submit">注销</button></td>'
							+ '</tr>';
				}
				$("table").html(str);
